...
country = {
	...
	UNCIV_TECH_SPREAD_MAX = 0, --Max techs an unciv will get on westernizing
	UNCIV_TECH_SPREAD_MIN = 0, --Minimum techs an unciv will get on westernizing
	...
	NEWSPAPER_PRINTING_FREQUENCY = 1780, -- days frequency when the news attempt to be printed (may not print if not enough facts collected, and retry after another X days)
	NEWSPAPER_TIMEOUT_PERIOD = 4450, -- the max period for news that may be printed. The probability is decreased with time.
...
economy = {
	...
	TRADE_CAP_LOW_LIMIT_NAVAL = 0.75, 			-- the lowest % the slider can go for naval units
...
military = {
	...
	SUPPLY_RANGE = 50,
	POP_MIN_SIZE_FOR_REGIMENT_PROTECTORATE_MULTIPLIER = 10,
	...
	NAVAL_LOW_SUPPLY_DAMAGE_MIN_STR = 0, -- when low supply, the navy will supply STR damage but no less then X% to avoid destruction (value from 0 to 100.0)
...
diplomacy = {
	...
	PEACE_COST_UNINSTALL_COMMUNIST_GOV_TYPE = 70,
	...
	WARGOAL_JINGOISM_REQUIREMENT = 0.000,

	LIBERATE_STATE_RELATION_INCREASE = 50,
	DISHONORED_CALLALLY_PRESTIGE_PENALTY = -3,
	BASE_TRUCE_MONTHS = 0,
	MAX_INFLUENCE = 100,
	WARSUBSIDIES_PERCENT = 1, -- How many percent of imports you are going to pay each day(mil. constructions and mil. maintainence)
...
ai =
{
	...
	PRODUCTION_WEIGHT = 0, -- ai weight for new production
